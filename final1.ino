...
char serverName[] = "10.58.17.144";    // name address of your domain
...
// leemos del pin A0 valor
  ...
  gas = map(valorGas, 0, 1023, -65, 255 ); //0-10 Normal 10 - 20 Advertencia 20 - 255 Peligro
...
void calculoLat(){ 
  ...
  lat = random(0, 100); //0-10 Normal 10 - 20 Advertencia 20 - 255 Peligro

// leemos del pin A5 valor
  //float lat = mq2.readLPG();
  Serial.println("Medicion de latitud: ");
  //Al método ftoa, se le envía el arreglo llamado tempString, el valor del sensor, cantidad de decimales
  Serial.println(ftoa(tempString,lat,0));
  //Se envian los datos a las API creadas en el servidor, asegurandose de que los datos sean los mismos que recibirá la página php
  //en la variable $arduino_data = $_GET['data_temperatura']; por ejemplo.
  sprintf(pageAdd,"/arduino/latitud.php?latValor=%s",ftoa(tempString,lat,0));
...
void calculoLong(){ 
  ...
  longitud = random(0, 100); //0-10 Normal 10 - 20 Advertencia 20 - 255 Peligro

// leemos del pin A5 valor
  //float longitud = mq2.readLPG();
  Serial.println("Medicion de longitud: ");
  //Al método ftoa, se le envía el arreglo llamado tempString, el valor del sensor, cantidad de decimales
  Serial.println(ftoa(tempString,longitud,0));
  //Se envian los datos a las API creadas en el servidor, asegurandose de que los datos sean los mismos que recibirá la página php
  //en la variable $arduino_data = $_GET['data_temperatura']; por ejemplo.
  sprintf(pageAdd,"/arduino/longitud.php?longValor=%s",ftoa(tempString,longitud,0));
